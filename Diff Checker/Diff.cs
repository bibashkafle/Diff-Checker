    ...
    public class diff_match_patch
    {
        ...
        public string diff_prettyHtml(List<Diff> diffs)
        {
            ...
            foreach (Diff aDiff in diffs)
            {
                ...
                {
                    case Operation.INSERT:
                        //html.Append("<ins style=\"background:#00FFFF;\">").Append(text).Append("</ins>"); //#e6ffe6
                        html.Append("<span style=\"background:#00FFFF;\">").Append(text).Append("</span>"); //#e6ffe6        
                        break;
                    case Operation.DELETE:
                        html.Append("<span style=\"background:#EE82EE;\">").Append(text).Append("</span>"); //#ffe6e6                            
                        break;
                    case Operation.EQUAL:
        ...
        public List<Diff> GetDifferent(List<Diff> diffs, bool isText1=true)
        {
            ...
            foreach (Diff aDiff in diffs)
            {
                if (isText1)
                {
                    if (aDiff.operation != Operation.INSERT)
                    {
                        diffList.Add(new Diff(aDiff.operation, aDiff.text));
                    }
                }
                else
                {
